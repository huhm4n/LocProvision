...
If ($SamOK) {
	...
	$h = echo $sam
	Copy-Item -Path c:\LocProvision\vagrant_template\ansible\* -Destination c:\LocProvision\vagrant_f\$h -recurse -Force
	(Get-Content c:\LocProvision\vagrant_f\$h\Vagrantfile) -replace "ansible.test",$h | out-file -Encoding utf8 c:\LocProvision\vagrant_f\$h\Vagrantfile
	(Get-Content c:\LocProvision\vagrant_f\$h\provision\set_hostname.sh) -replace "ansible.test",$h | out-file -Encoding utf8 c:\LocProvision\vagrant_f\$h\provision\set_hostname.sh
	$command = 'vagrant up'
